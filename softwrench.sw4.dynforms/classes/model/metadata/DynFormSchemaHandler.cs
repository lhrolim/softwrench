...
namespace softwrench.sw4.dynforms.classes.model.metadata {
    ...
    public class DynFormSchemaHandler : ISingletonComponent, ISWEventListener<RefreshMetadataEvent> {
        ...
        private ILog Logger = LogManager.GetLogger(typeof(DynFormSchemaHandler));
        ...
        public async Task ReplaceDetailDisplayables(string formName, string newFieldsJSON) {
            ...
            newDisplayables = RemoveDuplicateAutoGeneratedFields(newDisplayables);


            var linearDisplayable = DisplayableUtil.GetDisplayable<IApplicationIndentifiedDisplayable>(typeof(IApplicationIndentifiedDisplayable), newDisplayables).Where(x => !(x is ApplicationSection));
        ...
        private List<IApplicationDisplayable> RemoveDuplicateAutoGeneratedFields(List<IApplicationDisplayable> newDisplayables) {
            List<IApplicationDisplayable> results = new List<IApplicationDisplayable>();
            foreach (var displayable in newDisplayables) {
                var f = displayable as ApplicationFieldDefinition;
                if (f == null) {
                    results.Add(displayable);
                    continue;
                }

                if (f.AutoGenerated && results.Any(o => f.Role.Equals(o.Role))) {
                    Logger.WarnFormat("duplicate field {0} ignored", f.Role);
                    continue;
                }
                results.Add(displayable);
            }
