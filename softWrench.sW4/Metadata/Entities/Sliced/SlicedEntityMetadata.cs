...
namespace softWrench.sW4.Metadata.Entities.Sliced {
    public class SlicedEntityMetadata : EntityMetadata {
        ...
        public override ISet<EntityAssociation> NonListAssociations(bool innerCall = false) {
            ...
            if (!innerCall) {
                //excluding any entity relationships used by associations on the application level...
                //TODO: document this thing better
                var entityAssociationsNotUsedByAppAssociations = resultList.Where(NotDeclaredByAssociationNorByAutoGeneratedFilter());
        ...
        private Func<EntityAssociation, bool> NotDeclaredByAssociationNorByAutoGeneratedFilter() {
            return r => !_appSchema.Associations().Any(a => a.AssociationKey.EqualsIc(r.Qualifier) || a.AssociationKey.EqualsIc(r.To)) || _appSchema.HiddenFields.Any(h => !h.AutoGenerated && h.Attribute.Contains("_.")
            && EntityUtil.GetRelationshipName(h.Attribute).Equals(r.Qualifier));
        }
