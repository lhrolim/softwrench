...
namespace softWrench.sW4.Metadata.Applications.Schema {
    class ApplicationSchemaLazyFkHandler {
        ...
        public static ApplicationSchemaDefinition.LazyFkResolverDelegate LazyFkResolverDelegate = delegate (ApplicationSchemaDefinition definition) {
            ...
            var resultList = new HashSet<IApplicationDisplayable>();
            resultList.AddAll(DirectFKsFields(definition));
            resultList.AddAll(InverseFKsFields(definition.ApplicationName));
            if (definition.GetProperty(ApplicationSchemaPropertiesCatalog.OriginalApplication) != null) {
                resultList.AddAll(InverseFKsFields(definition.GetProperty(ApplicationSchemaPropertiesCatalog.OriginalApplication)));
            }
        ...
        private static IEnumerable<IApplicationAttributeDisplayable> InverseFKsFields(string applicationName) {
            ...
            var relationshipName = EntityUtil.GetRelationshipName(applicationName);
            ...
            foreach (var association in relationships) {
                ...
                foreach (var attribute in entityAssociation.Attributes) {
                    if (attribute.To != null) {
                        var applicationFieldDefinition = ApplicationFieldDefinition.HiddenInstance(applicationName, attribute.To);
