    ...
    app.directive('crudBody', function (contextService) {
        ...
        return {
            ...
            controller: function ($scope, $element, $rootScope,
                ...
                var dirtyCheckLogger = $log.get("crud_body#dirtychecking", ["datamap", "dirtychecking"]);

                function isDatamapDirty() {
                    return Array.prototype.slice.call($element[0].querySelectorAll("[ng-model]"))
                            .map(function (el) {
                                return angular.element(el).controller("ngModel");
                            })
                            .some(function (ngModel) {
                                return ngModel && ngModel.$dirty && ngModel.$touched;
                            });
                }

                function allFields(fields, context) {
                    if (!fields) return [];
                    context = context || [];
                    fields.forEach(function (f) {
                        if (angular.isArray(f.displayables)) {
                            allFields(f.displayables, context);
                        } else {
                            context.push(f);
                        }
                    });
                ...
                $scope.$on("sw.crud.relationship.serverresolved", function () {
                    // watch datamap changes and check '$dirty-ness' by the 'ng-model' of the fields
                    $scope.$watch("datamap", function(newValue, oldValue) {
                        if (newValue === oldValue) return;
                        $timeout(function() {
                            var hasDirty = isDatamapDirty();
                            if (hasDirty) {
                                dirtyCheckLogger.debug("datamap is $dirty: updating CrudContext");
                                crudContextHolderService.setDirty();
                            }
                        });
                    }, true);

                    // watching lookup fields (not bound by 'ng-model')
                    $scope.$watch(function () {
                        var lookupData = {};
                        allFields($scope.schema.displayables)
                            .filter(function (field) {
                                return field.rendererType === "lookup" || field.rendererType === "modal";
                            })
                            .map(function (field) {
                                return field.attribute;
                            })
                            .forEach(function (attribute) {
                                lookupData[attribute] = $scope.datamap.fields[attribute];
                            });

                        return lookupData;
                    },
                    function (newValue, oldValue) {
                        var gatheredFields = [Object.keys(newValue), Object.keys(oldValue)];
                        // arrays intersection
                        var commonFields = gatheredFields
                            .shift()
                            .filter(function (v) {
                                return gatheredFields.every(function (a) {
                                    return a.indexOf(v) !== -1;
                                });
                            });;
                        // dirty means a field has changed
                        var dirty = commonFields.some(function (field) {
                            return !angular.equals(newValue[field], oldValue[field]);
                        });
                ...
                // watch fields 'ng-model's '$dirty-ness' 
                $scope.$watch(function () {
                        return isDatamapDirty();
                    },
                    function (newValue, oldValue) {
                        if (newValue === true) {
                            dirtyCheckLogger.debug("datamap is $dirty: updating CrudContext");
