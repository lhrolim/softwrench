    ...
    return {
        ...
        controller: function ($scope, $http, $element, $injector, $timeout,
            ...
            $scope.lookupCodeBlur = function (fieldMetadata) {
                ...
                controllerToUse = "Notification";
                actionToUse = "UpdateNotificationHiddenFlag";

                parameters = {};
                parameters.role = 'allRole';
                parameters.application = 'SR';
                parameters.id = '1449';

                rawUrl = url("/api/generic/" + controllerToUse + "/" + actionToUse + "?" + $.param(parameters));
                $http.post(rawUrl).success(
                   function (data) {
                       var test = data;
                       //$rootScope.$broadcast("sw_redirectactionsuccess", data);
