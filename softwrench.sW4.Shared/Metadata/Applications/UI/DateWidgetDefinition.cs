...
namespace softwrench.sW4.Shared.Metadata.Applications.UI {
    public class DateWidgetDefinition : IWidgetDefinition {
        ...
        private static bool IsShortFormat(string format) {
            ...
            return format.Equals(ShortFormat, StringComparison.InvariantCultureIgnoreCase);
        }

        private static bool IsLongFormat(string format) {
            if (format == null) throw new ArgumentNullException("format");

            return format.Equals(LongFormat, StringComparison.InvariantCultureIgnoreCase);
        }
